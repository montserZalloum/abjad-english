...
FACTORY_REQUIRED_MARKERS="${FACTORY_REQUIRED_MARKERS:-APP_STARTED E2E_PASSED HOLDOUT_PASSED PROTECTED_OK GATE_OK}"
...
FACTORY_BUILD_INCLUDE="${FACTORY_BUILD_INCLUDE:-index.html src content styles assets scripts package.json}"
...
# Interview answer: macOS notification + a GitHub comment (the comment is posted by the
# workflow itself; this command is the local half). notify.sh reads stdin per the
# contract below and pops an osascript notification.
FACTORY_NOTIFY_CMD="${FACTORY_NOTIFY_CMD:-bash factory/notify.sh}"
